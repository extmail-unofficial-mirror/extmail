...
sub dump_parts {
    ...
    if($mode eq 'to_disk') {
        ...
        # XXX FIXME - if we can do chunk reading (eg: 32K or even 64K) it will
        # speed up downloading, but need much care on algorithm design
        my $rem = '';
        while(<$FD>) {
            if (length $rem) {
                $_ = $rem . $_;
...
sub decode_it {
    ...
    if ($type=~/base64/i) {
        ...
        if ($charset =~ /^(hz|hz-gb|hz-gb-2312)/i) {
            return decode_hz(decode_base64($s));
        }
        return decode_base64($s);
    }
    if ($type=~/quoted-printable/i) {
        if ($charset =~ /^(hz|hz-gb|hz-gb-2312)/i) {                        
            return decode_hz(decode_qp($s));
        }
        return decode_qp($s);
    }
    if ($type=~/7bit/) {
